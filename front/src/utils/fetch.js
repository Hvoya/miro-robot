import { getTokenFromLocalStorage } from './LocalStorageFunctions'
let loaderAction = undefined,
  requests = 0;
...
function request(uri, params = {}, data = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Set token header
    if (getTokenFromLocalStorage()) xhr.setRequestHeader("Authorization", getTokenFromLocalStorage());
...
function upload(uri, data) {
  return new Promise((resolve, reject) => {
    request(uri, Object.assign({ method: "POST", requestType: 'upload' }), data)
      .then(res => resolve(res.image ? res.image : res))
  })
}

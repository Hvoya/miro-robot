<template>
  <ProjectPageTemplate>
    <h3>{{$t('headings.user')}}</h3>
    <h2>
      <span class="red-text">{{user.username}}</span>
    </h2>
    <a-card>
      <a-row :gutter="30" :style="{marginBottom:'30px'}">
        <a-col :span="6">
          ...
          <div class="profile_avatar-wrapper">
            <img class="profile_avatar" :src="'/api/images/' + user.image.name" alt />
          </div>
        </a-col>
        <a-col :span="18">
      ...
      <a-row>
        <a-col :span="24">
          <h4 class="profile_sub-heaidng">{{$t('headings.user_projects')}}</h4>
          <a-table
            :dataSource="projects"
            :columns="columns"
